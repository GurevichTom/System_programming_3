...
void L_insertSorted(LIST *pList, DATA Value, int (*compare) (const void*, const void*)) {
	if (!pList || !compare) {
		return;
	}
...
/////////////////////////////////////////////////////////
const NODE* L_find(const NODE* pNode, const void* Value, int (*equals) (const void*, const void*))
{
	if (!pNode || !Value || !equals)
		return NULL;

	for (const NODE* tmp = pNode; tmp; tmp = tmp->next)
	{
		if (equals(tmp->key, Value))
			return tmp;
	}
